...
/**
 * Import a user pet: pick a PNG sprite sheet, describe its grid (frame size +
 * fps), map the five states to frame ranges, preview live, and save. Frame size
 * is the only thing the picker can't infer — the live preview makes it easy to
 * dial in (tweak until one clean frame shows).
 */
export const PetImport = ({ pickImage, onImport, onCancel }: PetImportProps): ReactElement => {
  ...
  const manifest: PetManifest = useMemo(
    () => ({
      name: name.trim() || pick?.fileName?.replace(/\.png$/i, "") || "My pet",
      frameWidth,
      frameHeight,
      columns,
      fps,
    ...
    [name, pick, frameWidth, frameHeight, columns, fps, ranges]
  );

  const previewPet: PetData | null = pick
    ? { id: "preview", name: manifest.name, manifest, sheetUrl: pick.dataUrl, custom: true }
    : null;
  ...
  const remap = (): void => setRanges(autoMapRows(columns, rows));
  ...
  const save = async (): Promise<void> => {
    ...
    setSaving(true);
    try {
      await onImport({ name: manifest.name, manifest, dataUrl: pick.dataUrl });
    } finally {
      setSaving(false);
    }
  };
    ...
    <div className="pet-import" aria-label="Import a pet">
      {!pick ? (
        <button type="button" className="pet-pick-btn" onClick={() => void choose()}>
          <ImagePlus size={15} /> Choose a sprite sheet (PNG)
        </button>
      ) : (
        <div className="pet-import-body">
          <div className="pet-import-preview">
            {previewPet && <Pet pet={previewPet} state={previewState} size={96} />}
          ...
          <div className="pet-import-fields">
            ...
            <div className="pet-import-ranges">
              <div className="pet-import-ranges-head">
                ...
                <button type="button" className="pet-remap" onClick={remap}>
